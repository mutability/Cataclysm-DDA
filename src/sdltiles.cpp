...
public:
     ...
     * using (curses) color.
     */
    virtual void OutputChar(std::string ch, int x, int y, unsigned char FG, unsigned char BG) = 0;
...
public:
    ...
    virtual void OutputChar(std::string ch, int x, int y, unsigned char FG, unsigned char BG) override;
...
static void report_gpu_errors( DebugLevel level = D_INFO )
{
...
void CachedTTFFont::OutputChar( std::string ch, int const x, int const y, unsigned char FG, unsigned char BG )
{
    if( !backgrounds[BG] ) {
